...
// 按页码查看
// 用法：
// 1 添加插件相应的css文件 pager.css
// 2 按顺序添加 jquery.js 和 pager.js
// 3 调用插件
// $pages.pager({
//  'start': 1,
//	'current': '1', // 默认选中页
...
;(function ($, win) {
    ...
    $.fn.pager = function pager(options) {

		var $pages = this/*.hide()*/; // 加载页码的父容器

		// 载入pageView.css - 太抖了，加入hide和show
		/*if (typeof pager.first === 'undefined') {
			win.legend.jquery.pluginUtil.insertCSS('css/pager.css');
			pager.first = false;
		}*/
		...
		for (var i = 1; i <= count; ++i) { 
			pageHtml += '<li class="page-num interactive num" data-num="'+i+'">'+i+'</li>';
		}
		pageHtml += '<li class="page-num interactive next">下一页</li></ul>';
		$pages[0].innerHTML = pageHtml;

		// 一次处理多个节点
		/*$pages.each(function (i, v) {
		    _pageView($(v), pageHtml);
		});*/
		...
		setCurrent(+settings.current);
		
		// 5 点击
		if (gTouchable) {
			// 5.1 点击上一页
			$pages.on('touchstart', '.prev.interactive', function () {
			    hilight($(this));
			});
			$pages.on('touchend', '.prev', function (e) {
			    dim($(this));
				prev(e);
			});

			// 5.2 点击下一页
			$pages.on('touchstart', '.next.interactive', function (e) {
				hilight($(this));			    
			});
			$pages.on('touchend', '.next', function (e) {
				dim($(this));
				next(e);
			});

			// 5.3 点击页码
			$pages.on('touchstart', '.num', changeTo);
		}
		else {
			$pages.on('click', '.prev', prev);
		    $pages.on('click', '.next', next);
			$pages.on('click', '.num', changeTo);
		}
		...
		var interfaces = {};
		interfaces.setCurrent = setCurrent;
		//return this; // 保持链式不中断
		return interfaces;
	};

	function _pageView($page, html) {
